...
def streamlit_config():
    ...
    # page configuration
    st.set_page_config(page_title="BCP CV AI Analyzer", layout="wide")
...
# title and position
st.markdown(
    f'<h1 style="text-align: center;">BCP AI CV Analyzer</h1>',
        ...
        if submit:
            if pdf is not None:
                try:
                    ...
                    st.markdown(
                        f'<h4 style="color: orange;">Summary:</h4>',
                        unsafe_allow_html=True,
                    )
                    st.write(summary)
        ...
        if submit:
            if pdf is not None and openai_api_key != "":
                try:
                    ...
                    st.markdown(
                        f'<h4 style="color: orange;">Strength:</h4>',
                        unsafe_allow_html=True,
                    )
                    st.write(strength)
        ...
        if submit:
            if pdf is not None and openai_api_key != "":
                try:
                    ...
                    st.markdown(
                        f'<h4 style="color: orange;">Weakness and Suggestions:</h4>',
                        unsafe_allow_html=True,
                    )
                    st.write(weakness)
...
class linkedin_scraper:
        ...
        with st.form(key="linkedin_scarp"):
            ...
            with col3:
                job_count = st.number_input(
                    label="Job Count", min_value=1, value=1, step=1
                )
    ...
    def open_link(driver, link):
        ...
        while True:
            ...
            try:
                ...
                driver.find_element(
                    by=By.CSS_SELECTOR,
                    value="span.switcher-tabs__placeholder-text.m-auto",
                )
                return
    ...
    def display_data_userinterface(df_final):
        ...
        if len(df_final) > 0:
            for i in range(0, len(df_final)):
                ...
                st.markdown(
                    f'<h3 style="color: orange;">Job Posting Details : {i+1}</h3>',
                    unsafe_allow_html=True,
                )
            ...
            st.markdown(
                f'<h5 style="text-align: center;color: orange;">No Matching Jobs Found</h5>',
                unsafe_allow_html=True,
            )
    ...
    def main():
        ...
        try:
            job_title_input, job_location, job_count, submit = (
            ...
            if submit:
                if job_title_input != [] and job_location != "":
                    ...
                    with st.spinner("scraping Job Details..."):
                        ...
                        df = linkedin_scraper.scrap_company_data(
                            driver, job_title_input, job_location
                        )

                        # Scraping the Job Descriptin Data
                        df_final = linkedin_scraper.scrap_job_description(
                            driver, df, job_count
                        )
                ...
                # If User Click Submit Button and Job Title is Empty
                    ...
                    st.markdown(
                        f'<h5 style="text-align: center;color: orange;">Job Title is Empty</h5>',
                        unsafe_allow_html=True,
                    )

                elif job_location == "":
                    st.markdown(
                        f'<h5 style="text-align: center;color: orange;">Job Location is Empty</h5>',
                        unsafe_allow_html=True,
                    )
